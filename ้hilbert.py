...
class Hilberts:
    ...
    def write_to_file(self, filename):
        with open(filename, 'w', newline='') as file:
            ...
            # Create a list of rows to write in bulk
            rows = []

            for channel, base in self.channels.items():
                rows.extend([[base ** i, f"channel {channel}"] for i in range(1, self.guests_per_channel[channel] + 1)])

            rows.extend([[room, "manual"] for room in self.manual_rooms])
